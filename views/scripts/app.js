    ...
    fetch(`/apod?date=${date}`).then((response) => {
      response.json().then((data) => {
        if (data.error) {
          ...
          msgOne.textContent =
            "This page shows the Astronomy Picture of the Day!";
...
if (neoForm) {
  neoForm.addEventListener("submit", (e) => {
    ...
    fetch(`/neo?start_date=${startDate}&end_date=${startDate}`).then(
      (response) => {
        response.json().then((data) => {
          if (data.error) {
            ...
            msgTwo.textContent =
              "This page shows the Near Earth Objects by Date";
            neoUL.innerHTML = "";
            data.data.list.forEach((neo) => {
              ...
              neoHeader.appendChild(
                document.createTextNode(
                  `Asteroid Name: ${neo.neoName
                    .replace("(", "")
                    .replace(")", "")}`
                )
              );
              ...
              neoSpanLI.appendChild(
                document.createTextNode(
                  `Potentially Hazardous: ${neo.hazardous}`
                )
              );
              ...
              neoSpanLI.appendChild(
                document.createTextNode(
                  `Diameter: ${Math.round(
                    neo.diameter.estimated_diameter_min
                  )} to ${Math.round(
                    neo.diameter.estimated_diameter_max
                  )} meters (approx.)`
                )
              );
              neoSpanUL.appendChild(neoSpanLI);
              ...
              neoSpanLI.appendChild(
                document.createTextNode(
                  `Date of Closest Approach: ${neo.closeApproachDate}`
                )
              );
              neoSpanUL.appendChild(neoSpanLI);
              ...
              neoSpanLI.appendChild(
                document.createTextNode(
                  `Relative Velocity: ${Math.round(
                    neo.velocity.kilometers_per_hour
                  )} km/h (approx.)`
                )
              );
              neoSpanUL.appendChild(neoSpanLI);
              ...
              neoSpanLI = document.createElement("li");
              neoSpanLI.appendChild(
                document.createTextNode(`NASA - JPL URL: `)
              );
